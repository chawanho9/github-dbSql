1. PRIMARY KEY 제약 조건 생성시 오라클 dbms는 해당 컬럼으로 unique index 를 자동으로 생성한다.
   (***정확히는 UNIQUE 제약에 의해 UNIQUE 인덱스가 자동으로 생성된다.
        PRIMARY KEY = UNIQUE + NOT NULL )
index : 해당 컬럼으로 미리 정렬을 해놓은 객체
정렬이 되어 있기 떄문에 찾고자 하는 값이 존재하는지 빠르게 알수가 있다.
만약에 인덱스가 없다면 새로운 데이터를 입력할 때  중복되는 값을 찾기 위해서 테이블의 모든 데이터를
찾아야 한다. 하지만 인덱스가 있으면 이미 정렬이 되어 있기 떄문에 해당 값의 존재 유무를 빠르게 알수있다.
...
FOREIGN KEY 생성시 옵션
FOREIGN KEY (참조 무결성) : 참조하는 테이블의 컬럼에 존재한느 값만 입력 될 수 있도록 제한
...
(EX : EXMP.deptno  ==> DEPT.deptno 컬럼을 참조 하고 있을 때
...
SELECT * 
FROM emp_test;
--테이블 생성 순서를 고려함 , 데이터를 어떤 순서로 넣어야할지 고려함 , 삭제 순서도 고려해야함

FOREIGN KEY 옵션
1. ON DELETE CASCADE : 부모가 삭제될 경우 (dept) 참조하는 자식 데이터도 같이 삭제한다(emp);
2. ON DELETE SET NULL : 부모가 삭제될 경우 (dept) 참조하는 자식 데이터의 컬럼을 NULL 로 설정;

emp_test테이블을 DROP 후 옵션을 번갈아 가며 생성후 테스트;
...
옵션 부여하지 않았을떄는 위의 delete 쿼리가 에러가 발생;
옵션에 따라서 참조하는 자식테이블의 데이터가 정상적으로 삭제가 되었는데 SELECT 확인;

SELECT *
FROM emp_test;

FK ON DELET SET NULL 옵션 테스트;
...
부모 테이블의 데이터 삭제 후 자식 테이블의 데이터가 NULL 로 변경되었는지 확인;

SELECT * 
FROM emp_test;

CHECK 제약조건 : 컬럼에 들어가는 값의 종류를 제한할 떄 사용
ex : 급여 컬럼을 숫자로 관리, 급여가 음수가 들어갈 수 있을까?;
     일반적인 경우 급여값은 > 0
     급여값이 0보다 큰 값인지 검가사능 EMP테이블의 job 컬럼에 들어가는 값을 다음 4가지로 제한가능 
...
emp 테이블을 이요앻서 부서번호가 10인 사원들만 조회하여 해당 데이터로 
emp_test2 테이블을 생성;
...
--1.컬럼 추가
ALTER TABLE 테이블명 ADD (신규컬럼명 신규 컬럼 타입);
...
ALTER TABLE 테이블명 MODIFY (기존 컬럼명 신큐 컬럼 타입 (사이즈));
...
emp_test에 hp_a 컬럼이 있는 것을 확인;
SELECT*
FROM emp_test;


ALTER TABLE emp_test DROP COLUMN hp_n;
emp_test에 hp_a 컬럼이 삭제되어 있는지 확인 
SELECT*
FROM emp_test;
...
--defult전에 값이 미리 들어가있으면 deulft 이후에 바뀌지 않음
ALTER TABLE emp_test RENAME COLUMN hp TO hp_n;

TABLE 변경 : 6. 제약 조건 추가 /삭제;
...
PRIMARY KEY 테스트;
INSERT INTO emp_test VALUES (9999,'brown',99);
INSERT INTO emp_test VALUES (9999,'sally',99); 첫번쨰 insert 구문에 의해 중복되므로 실패;

FOREIGN KEY 테스트;
INSERT INTO emp_test VALUES (9998,'sally',98); dept_test 테이블에 존재하지 않는 부서번호이므로 실패;

제약조건 삭제 : PRIMARY KEY,FORIGN KEY;
...
--중복된 empno값으로 데이터입력
INSERT INTO emp_test VALUES (9999,'brown',99);
INSERT INTO emp_test VALUES (9999,'sally',99); 첫번쨰 insert 구문에 의해 중복되지만 성공;

--존배하지 않는 98번부서로 데이터 입력
...
1.emp_test 테이블 삭제
2.emp_test 테이블 생성
3.ALTER TABLE PRIMARY KEY(empno), FOREIGN KEY(dept_test) 제약조건 생성
4.두개의 제약조건을 비활성화
...
emp_test 테이블에는 empno컬럼의 값이 9999인 사원이 두명 존재하기 때문에
...
dept_test 테이블에 존재하지 않는 부서번호98을 emp_test에서 사용중이라 오류가 뜸
